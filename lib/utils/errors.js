 ...
 *   └── ListenerError    - Payload/callback listener errors (bind, server stopped)
 */
...
/**
 * Listener error - payload/callback listener failed to bind or was stopped
 * while a caller was still waiting for a hit (e.g. EADDRINUSE, EACCES,
 * stop() called during waitForHit()).
 * @param {string} message - Error message
 * @param {string} [code='LISTENER_ERROR'] - Error code (e.g. underlying socket code)
 * @param {Error|null} [cause=null] - Original error
 */
export class ListenerError extends JsploitError {
    constructor(message, code = 'LISTENER_ERROR', cause = null) {
        super(message, code);
